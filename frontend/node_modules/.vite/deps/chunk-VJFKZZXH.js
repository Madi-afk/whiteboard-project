...
} from "./chunk-FGAWXSH6.js";
import {
  at
} from "./chunk-KYLTHSDW.js";
import {
  compileStyles,
  solidStateFill,
  styles2String,
  userNodeOverrides
} from "./chunk-A3RWWFAI.js";
import {
  createText,
  getIconSVG
...
async function curvedTrapezoid(parent, node) {
  ...
  const { shapeSvg, bbox } = await labelHelper(parent, node, getNodeClasses(node));
  const w = Math.max(minWidth, (bbox.width + labelPaddingX * 2) * 1.25, (node == null ? void 0 : node.width) ?? 0);
...
async function doublecircle(parent, node) {
  ...
  const { shapeSvg, bbox } = await labelHelper(parent, node, getNodeClasses(node));
  const outerRadius = ((node == null ? void 0 : node.width) ? (node == null ? void 0 : node.width) / 2 : bbox.width / 2) + (labelPadding ?? 0);
...
async function halfRoundedRectangle(parent, node) {
  ...
  const { shapeSvg, bbox } = await labelHelper(parent, node, getNodeClasses(node));
  const w = ((node == null ? void 0 : node.width) ? node == null ? void 0 : node.width : Math.max(minWidth, bbox.width)) + paddingX * 2;
...
async function inv_trapezoid(parent, node) {
  ...
  const labelPaddingX = node.look === "neo" ? nodePadding * 2 : nodePadding;
  const { shapeSvg, bbox } = await labelHelper(parent, node, getNodeClasses(node));
  const w = Math.max(bbox.width + (labelPaddingX ?? 0) * 2, (node == null ? void 0 : node.width) ?? 0);
  const h = Math.max(bbox.height + (labelPaddingY ?? 0) * 2, (node == null ? void 0 : node.height) ?? 0);
  const points = [
    { x: 0, y: 0 },
    { x: w, y: 0 },
    { x: w + 3 * h / 6, y: -h },
...
async function lean_left(parent, node) {
  ...
  const labelPaddingX = node.look === "neo" ? nodePadding * 2 : nodePadding;
  const { shapeSvg, bbox } = await labelHelper(parent, node, getNodeClasses(node));
  const h = ((node == null ? void 0 : node.height) ?? bbox.height) + labelPaddingY;
  const w = ((node == null ? void 0 : node.width) ?? bbox.width) + labelPaddingX;
  const points = [
    { x: 0, y: 0 },
    { x: w + 3 * h / 6, y: 0 },
    { x: w, y: -h },
...
async function multiWaveEdgedRectangle(parent, node) {
  ...
  label.attr(
    "transform",
    `translate(${-(bbox.width / 2) - rectOffset2 - (bbox.x - (bbox.left ?? 0))}, ${-(bbox.height / 2) + rectOffset2 - waveAmplitude / 2 - (bbox.y - (bbox.top ?? 0))})`
  );
  updateNodeBounds(node, shape);
  node.intersect = function(point) {
    const pos = intersect_default.polygon(node, outerPathPoints, point);
...
async function rect_left_inv_arrow(parent, node) {
  ...
  const { shapeSvg, bbox, label } = await labelHelper(parent, node, getNodeClasses(node));
  const w = ((node == null ? void 0 : node.width) ?? bbox.width) + (node.look === "neo" ? labelPaddingX * 2 : labelPaddingX);
  const h = ((node == null ? void 0 : node.height) ?? bbox.height) + (node.look === "neo" ? labelPaddingY * 2 : labelPaddingY);
  const x = -w / 2;
  const y = -h / 2;
  const notch = y / 2;
...
async function subroutine(parent, node) {
  ...
  const labelPaddingY = node.look === "neo" ? 12 : nodePadding;
  const { shapeSvg, bbox } = await labelHelper(parent, node, getNodeClasses(node));
  const totalWidth = ((node == null ? void 0 : node.width) ?? bbox.width) + 2 * FRAME_WIDTH2 + labelPaddingX;
  const totalHeight = ((node == null ? void 0 : node.height) ?? bbox.height) + labelPaddingY;
...
//# sourceMappingURL=chunk-VJFKZZXH.js.map
